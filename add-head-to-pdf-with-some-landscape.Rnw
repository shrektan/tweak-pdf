...
\includepdf[pages=1-3,
  ...
  width=1.3\textwidth,height=1.1\textheight,keepaspectratio,offset=0.0in -0.9in
]{%s}
)"
...
\headwidth=0.95\textheight

% \begin{landscape}
<<mergepdf2,results='asis',echo=FALSE>>=
cmd <- r"(
\includepdf[pages=4-,landscape=false,
  pagecommand=\setlayout{\fangsong %s}{}{},
  width=1.8\textwidth,height=1.5\textheight,keepaspectratio,offset=0.1in -0.8in
]{%s}
)"
cat(sprintf(cmd, title, normalizePath(file, winslash = "/")))
@
